...
export default class CallExpressionRuntime {
  public static evaluate(
    ...
    if (fn.kind === "native-fn") {
      return this.evaluate_function_native(stmt, fn, self);
    } else if (fn.kind === "function") {
      return this.evaluate_function(stmt, fn, self);
    }
  ...
  private static evaluate_function_native(
    ...
    const args = stmt.args.map((arg) => self.evaluate(arg));
  ...
  private static evaluate_function(
    ...
    // Validation and binding of arguments to parameters
    for (let i = 0; i < args.length; i++) {
  ...
  private static evaluate_lambda(
    ...
    // Checks if any external variable conflicts with the parameters
